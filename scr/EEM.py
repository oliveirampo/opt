...
class EEM(ABC):
    ...
    def computeEEM(self, cg, atomTypes, mol):
        ...
        for i in range(len(cg)):
            ...
            for j in range(i + 1, len(cg)):
                ...
                distance = mol.get_bond_distance(idx1, idx2)
                coul = self.coulomb(distance, sig1, sig2)
                X[i, j] = coul
                X[j, i] = coul
    ...
    def coulomb(self, distance, sig1, sig2):
        sig_sum = sig1 + sig2
        if distance == 0:
            return 0.0
        elif sig_sum == 0:
        ...
        return math.erf(distance / (math.sqrt(sig1 * sig1 + sig2 * sig2))) * 1.0 / distance
